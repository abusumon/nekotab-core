...
class PromoCodesView(SuperuserRequiredMixin, TemplateView):
    """Mint and review one-time promo codes.

    A promo code unlocks one tournament outright — it is the 100%-off route,
    with no payment processor involved at all. Two ways in: approving a bKash
    payment mints one automatically, or you generate them here to give away.

    Both kinds live in the same table and are told apart by whether they carry
    a bKash request, so "gifted" never gets counted as revenue.

    Not cached: it lists codes you have just created, and a stale page would
    hide them.
    """
    template_name = 'analytics/promo_codes.html'
    MAX_PER_BATCH = 100

    def get_context_data(self, **kwargs):
        from donations.models import PromoCode

        context = super().get_context_data(**kwargs)
        qs = PromoCode.objects.select_related(
            'request', 'used_for_tournament', 'used_by').order_by('-created_at')

        show = (self.request.GET.get('show') or 'all').strip()
        if show == 'unused':
            qs = qs.filter(used=False)
        elif show == 'used':
            qs = qs.filter(used=True)
        elif show == 'gifted':
            qs = qs.filter(request__isnull=True)

        context['codes'] = qs[:200]
        context['show'] = show
        context['total'] = PromoCode.objects.count()
        context['unused'] = PromoCode.objects.filter(used=False).count()
        context['used_count'] = PromoCode.objects.filter(used=True).count()
        context['gifted'] = PromoCode.objects.filter(request__isnull=True).count()
        context['max_batch'] = self.MAX_PER_BATCH
        # Codes generated by the POST that redirected here, shown once so they
        # can be copied. Held in the session rather than the querystring: a
        # promo code in a URL ends up in logs and browser history.
        context['just_created'] = self.request.session.pop('promo_just_created', [])
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            count = 0
        if count < 1 or count > self.MAX_PER_BATCH:
            messages.error(request, 'Choose a quantity between 1 and %d.' % self.MAX_PER_BATCH)
            return redirect('analytics:promo_codes')

        note_email = (request.POST.get('issued_to_email') or '').strip()

        created = []
        for _i in range(count):
            code = PromoCode.generate_code()
            if code is None:
                logger.error('Ran out of unique promo codes after %d of %d', len(created), count)
                messages.error(
                    request,
                    'Generated %d of %d before running out of unique codes. Try again.'
                    % (len(created), count))
                break
            # request stays NULL: that is exactly what marks a code as gifted
            # rather than paid, and it is what the reporting split reads.
            created.append(PromoCode(code=code, issued_to_email=note_email))

        if created:
            PromoCode.objects.bulk_create(created)
            request.session['promo_just_created'] = [c.code for c in created]
            messages.success(
                request,
                'Created %d promo code%s. Copy them now — they are shown once.'
                % (len(created), '' if len(created) == 1 else 's'))
        return redirect('analytics:promo_codes')


class DeletePromoCodesView(SuperuserRequiredMixin, View):
    """Revoke unused promo codes.

    POST only, so a crawler following a link cannot destroy codes.

    Refuses to touch used codes: deleting one would erase the audit trail
    linking a redemption to the tournament it unlocked, and it would not
    re-lock that tournament anyway.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not codes:
            messages.error(request, 'No codes selected.')
            return redirect('analytics:promo_codes')

        deleted, _ = PromoCode.objects.filter(code__in=codes, used=False).delete()
        skipped = len(codes) - deleted
        if deleted:
            messages.success(request, 'Revoked %d unused code%s.'
                             % (deleted, '' if deleted == 1 else 's'))
        if skipped > 0:
            messages.error(request, '%d code%s already used and left untouched.'
...
class MonetizationSettingsView(SuperuserRequiredMixin, TemplateView):
    ...
    PRESETS = {
        ...
        'free_with_ads': {
            'label': 'Free tournaments, show ads',
            'ads_enabled': True,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if preset:
            ...
            for name in ('ads_enabled', 'premium_gate_enabled'):
