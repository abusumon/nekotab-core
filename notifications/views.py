    ...
    def form_valid(self, form: TestEmailForm) -> 'HttpResponseRedirect':
        ...
        try:
            ...
            if e.smtp_code == 550 and "Sender Identity" in smtp_error:
                messages.warning(self.request, _("Hint: Your SMTP provider may require a verified sender identity. "
                    "Ensure DEFAULT_FROM_EMAIL (or its domain) is verified in your provider "
                    "dashboard, then retry."))
                logger.warning("Suspected sender identity verification error in test email", exc_info=True)
            else:
